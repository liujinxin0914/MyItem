...
let WpFileUploadEvent = {
    ...
    "initWithDeleteFile" : function (wfu) {
        let uploadId = wfu.uploadId;
        let fileItemDeleteBt = $("#"+uploadId+" .fileItem .status i");
     ...
     */
    "showFileResult" : function(fileUrl,fileId,deleteFile,deleteEvent){
        // 获取对象，作为全局变量，方便修改
        let wfu = this;
        // 上传的Id
        let uploadId = wfu.uploadId;
        ...
        // 根据文件地址获取文件名称
        let fileName = WpFileUploadTools.getFileNameWithUrl(fileUrl);
        // 文件类型
        ...
        // 获取回显文件模版
        let modelStr = WpFileUploadViewsModel.getFileItemResultModel(fileType, fileId, fileName, isImg, fileUrl, deleteFile);
        $(boxJsObj).append(modelStr);
        // 如果删除文件，则添加删除文件的事件
        if(deleteFile) {
            $("#"+uploadId+" .box .fileItem[showFileCode='" + fileId + "']").mousedown(function () {
    ...
    "removeShowFileItem" : function (fileId) {
        ...
        // 上传的Id
        let uploadId = wfu.uploadId;
        let fileitemObj = $("#"+uploadId+" .box .fileItem[showFileCode='" + fileId + "']");
        fileitemObj.remove();
    }
...
let WpFileUploadViewsModel = {
    ...
    "getHeadButtonsView" : function(wfu){
        ...
        if(!wfu.isHiddenUploadBt){
            ...
            btsStr += '<i style="font-size: 20px">上传</i>';
        ...
        if(!wfu.isHiddenCleanBt){
            ...
            btsStr += '<i style="font-size: 20px">清除全部</i>';
     ...
     */
    "getFileItemResultModel" : function (fileType, fileId, fileName, isImg, fileUrl, deleteFile) {
        //默认显示类型
        let showTypeStr = '<div class="fileType">'+fileType+'</div> <i class="iconfont icon-wenjian"></i>';
        //是否是文件
        if (isImg) {
    ...
    "startUploadFile" : function (wfu,formData,rememberFile) {
        $.ajax({
            ...
            success:function(data){
                ...
                if(data!=null){
                    for (var i=0;i<15;i++){
                        if(data[i]!=undefined&&null!=data[i]&&data[i]!=""&&""!=data[i].trim()){
                            alert(data[i]);
                        }
                    }
                }
...
let WpFileUploadTools = {
    ...
    "setFileItemProgress" : function (wfu, fileCodeId, percent, status) {
        ...
        let uploadId = wfu.uploadId;
        // 文件进度
        let fileItemProgress = $("#"+uploadId+" .box .fileItem[fileCodeId='"+fileCodeId+"'] .progress>div");
        // 文件状态
        let fileItemStatus = $("#"+uploadId+" .box .fileItem[fileCodeId='"+fileCodeId+"'] .status>i");
        if (status == 1) {
            if (wfu.showFileItemProgress) {
    ...
    "uploadError":function(){
       let wfu = this;
        ...
        // 是否显示单个文件进度条
        if (wfu.showFileItemProgress) {
            let progressObj = $("#"+uploadId+" .box .fileItem .progress>div");
